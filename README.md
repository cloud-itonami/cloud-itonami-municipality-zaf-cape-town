# cloud-itonami-municipality-zaf-cape-town

Municipal-ordinance compliance catalog for **Cape Town** — the
TWENTY-SECOND municipality-level entry alongside
...
South Africa has three capitals — Pretoria (administrative, already
recorded at the country level in `cloud-itonami-iso3166-zaf`), Cape
Town (legislative — seat of Parliament), and Bloemfontein (judicial).
This repo covers Cape Town on the strength of its own City government.

## Scope

A **read-only reference/archive** catalog — not an Advisor⊣Governor
actuation actor. It proposes or executes nothing on the City of Cape
Town's behalf.
...
WebFetch entirely at the domain root — abandoned without forcing it.
Both entries here instead cite official
[capetown.gov.za](https://www.capetown.gov.za/) pages, which rendered
fully: the **City of Cape Town Municipal Planning Amendment By-law,
2025** (published in Western Cape Provincial Gazette Extraordinary No.
9117, 2025-08-08) and the **City Ombudsman By-law, 2025** (title and
year confirmed on the City's own by-laws index page; no more specific
date is stated there, so `:ordinance/enacted-date` is deliberately
year-only rather than invented).

## License

AGPL-3.0-or-later (matches the `cloud-itonami-iso3166-*` /
`-municipality-*` / `-assoc-*` / `-lei-*` convention). Ordinance text
itself remains the City of Cape Town's; this repo stores only citation
